...
Theorem add_inc_count : forall b : bag, forall n : nat, count n (add n b) = (count n b) + 1.
Proof.
intros b. induction n as [| n' IHn'].
...
Proof.
intros f H. intros n1 n2 H2. replace n1 with (f (f (n1))). rewrite H. rewrite <- H2. reflexivity.
...
Theorem rev_injective : forall (l1 l2 : natlist),
  rev l1 = rev l2 -> l1 = l2.
Proof.
  intros l1 l2 H. destruct l1 as [| h t].
...
(** How _many_ elements does the type [baz] have? (Explain in words,
    in a comment.) *)
(* Can it even support a single element? There's no base case it will use up all memory to find a single elemetn *)
(** [] *)
