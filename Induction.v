...
(** **** Exercise: 1 star, standard, optional (destruct_induction)
    ...
    destruct is used to create cases for a proof by cases and can also be used for forall statements with countably, which works if our cases are finite.
    induction is used to solve forall statements when the types are countably infinite.
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Theorem: Addition is commutative. For any Natural numbers [n], [m]
    n + m = m + n. 

    Proof: By induction on n,
    - First suppose that [n=0]. We show
      0 + m = m + 0.  
      0 + m = m by left associativity of 0. 
      m + 0 = m right associativity of 0 
    - Now suppose [n = Sn'] assuming forall m. n' + m = m + n' 
      (Sn') + m  = m + (Sn')
      By Theorem,       
      Sn' + m = S(m + n')
...
induction n as [|n' IHn'].
  ...
  - simpl. assert (H: forall o p: nat, o * S p = o + o * p).
    + induction o as [| o' IHo'].
      * intros p. simpl. reflexivity.
      * intros p. simpl. rewrite IHo'. rewrite add_assoc. rewrite add_assoc.
        assert (H2: p + o' = o' + p). 
        -- rewrite add_comm. reflexivity.
...
THIS IS WRONG, BUT THEN WHY?
If the type takes values over a finite set we can use destruct.
If the type takes values over a countably infinite set (it's a recursive data type) we need
to use induction.
...
(* Destruct should work simply because b is over a finite set*)
...
Proof.
  ...
  + intros m p. simpl.  rewrite IHn'. rewrite add_assoc. reflexivity.
Qed.
...
Proof.
  ...
  + reflexivity.
  + simpl. reflexivity.
  + simpl. rewrite add_assoc. rewrite IHb'. 
    replace (bin_to_nat b'' + 0) with (bin_to_nat b'').
    rewrite -> plus_n_Sm. rewrite -> plus_n_Sm. simpl. rewrite <- add_assoc. 
    replace (S (bin_to_nat b'') + 0) with (S (bin_to_nat b'')).
...
(** Fixpoint shift_right (n : nat) : nat := 
  match n with
    | 0 => 0
    | 1 => 0
    | S n' => 1 + (shift_right (minus n' 1))
  end.
Compute (minus 8 (shift_right 8)).

Fixpoint nat_to_bin (n:nat) : bin
 := match n with 
    | 0 => Z
    | S n' => if (even (S n')) then B0 (nat_to_bin(minus n' (shift_right (n')))) else
            B1 (nat_to_bin(minus n' (shift_right (n'))))
    end.
*)
...
Proof.
   induction n as [| n' IHn'].
   reflexivity. replace (nat_to_bin(S n')) with (incr(nat_to_bin n')).
   rewrite bin_to_nat_pres_incr. rewrite IHn'. reflexivity.
   destruct n' as [ | ]. reflexivity. reflexivity.
Qed.
(** [] *)
